"""Task-agnostic survey verifier.

Every survey task's verifier does the same work: read ``survey_result.json``,
validate the answer envelope and trajectory, then emit one
``question_response`` context per answer plus a ``trial_summary``. Nothing in
that is task-specific — the field kind for each answer comes from the
questionnaire's own ``questionType`` on the ``ask_question`` trajectory event,
not from the question's subject matter.

This copy is the canonical one. ``adhoc_survey_task.py`` copies it into every
generated ad-hoc task so a question asked at runtime gets the same verified
answer envelope, and the same aggregation contexts, as a hand-authored task.

A task that needs instrument-specific checks or derived constructs (see
``survey_us-economic-pressure``) still writes its own verifier.
"""
